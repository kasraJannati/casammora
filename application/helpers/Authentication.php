...
class Authentication {
	...
	public function login($username, $password) {
		...
		$sql = "SELECT * FROM " . $this->config['user_table'] . " WHERE username = '" . $username . "' AND password = '" . sha1($password) . "' AND active_flag = 1";
		...
		if ($this->db->row_count($sql)) {
			...
			$this->session->set('user_group_name', $result['user_group_name']);
			$this->session->set('user_group_id', $result['user_group_id']);

			$this->session->set('user_email', $result['username']);
